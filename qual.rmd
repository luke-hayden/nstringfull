...
ggplot(mmd2, aes(y=absnormresid, x=expression_b))+
  geom_point(alpha=0.1, colour="red2")+
  stat_smooth()+
...
# geom_hex()+
  ...
  scale_x_log10()+
  xlab("Expression Replicate B")+
  ylab("Absolute Normalised Residual")+
 ggtitle("How much expression needed for replicability? ", subtitle = "All genes, samples with replicates")
```
...
ggplot(mmd2, aes(y=absnormresid, x=expression_b,   label=ma ))+
  geom_point(aes(colour=ma))+
  stat_smooth()+
  geom_vline(xintercept=5, linetype=2)+
...
ggplot(data = hkdat, aes(Var1, Var2, fill = value, label=value))+
