...
#tunegrid <- expand.grid(mtry=16,  splitrule="extratrees",  min.node.size=7)
...
rfagemodel <- train(
  ...
  tuneGrid=tunegrid,
  trControl = trainControl(method = "cv", number = 50, verboseIter = T)
)
...
md$rfagepred <- predict(currbest, newdata=md)


md$intrain <- rownames(md) %in% rownames(rfagemodel$trainingData)
...
# scale_colour_manual(values=c("darkviolet", "cornflower blue", "red3"))+
  ...
  ggtitle("Using Marker gene expression to predict age", subtitle=paste("R^2 of correlation between measure from length and from gene expression:", round(summary(accrf)$r.squared, 3)))#+  facet_wrap(~intrain)
