...
md <-subset(md, rownames(md)%in% qualsum$sample[qualsum$good2])
...
#ggsave(plot=p,height=8,width=12,dpi=200, filename=paste("gbmfullqualinc.pdf"), useDingbats=FALSE, limitsize = FALSE)
```
...
ggplot(md, aes(x= group, y=resid, fill=group)) +
  geom_boxplot()+
  theme_bw()+ 
  xlab("" ) +
  ylab("")
...
ggplot(md, aes(x=predage, y=gbmagepred,colour=group))+
  geom_point()+
  xlab("Age (weeks) from body length") +
  ylab("Age (weeks) from marker gene expression")+
  theme_bw()
...
ggplot(md, aes(y= total,x=resid, colour=group)) +
  geom_point()+
  theme_bw()+ 
  xlab("" ) +
  ylab("")
