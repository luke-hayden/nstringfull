...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
quantcomp <- left_join(quanthk, hkdat[6:10], by="sample")
mquantcomp <- gather(quantcomp, key="threshold", value="numhk", over1, over3,over5,over10)
...
mhkall <- left_join( select(mhkall, sample, jsdres), chimhkall, by="sample")%>%
  ...
  left_join(hkdat[,6:10], by="sample")
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
mmhkall<- gather(mhkall, key="threshold", value="numhk", over1, over3,over5,over10)
  
mmhkall$threshold <- factor(mmhkall$threshold, levels= c("over1", "over3","over5","over10"))
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
qualsum <- select(mhkall, sample, chistat, jsdres, meanpred, over1, over3, over5, over10, flag)

qualsum$good <- qualsum$chistat <1000& qualsum$over3 >3 & qualsum$flag ==F
