...
md <- subset(md, rownames(md)%in% qualsum$sample[qualsum$over3 >= 4 & qualsum$chistat <500])

tr%in% qualsum$sample[qualsum$over3 >= 4 & qualsum$chistat <500]
...
mdhk <-    select(md, over3, group, ty ) %>%
  spread(value= over3, key=ty)
...
md2$good <- paste0(md2$group, "un") %in% qualsum$sample[qualsum$chistat < chithresh] &
            paste0(md2$group, "un") %in% qualsum$sample[qualsum$over3 >= overthresh] &
            paste0(md2$group, "reg") %in% qualsum$sample[qualsum$chistat < chithresh] &
            paste0(md2$group, "reg") %in% qualsum$sample[qualsum$over3 >= overthresh] 
...
ggsave(plot=p,height=7,width=10,dpi=200, filename=paste("rfwregen.png"),device="png",  limitsize = FALSE)
