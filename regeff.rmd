...
md <- left_join(md, sampleinfo, by="sample")# %>%  filter(!(is.na(predage)))
...
md2 <-  spread(md2, value=rfagepred,key=ty)

md2$dir <- md2$un > md2$re
md2$diff <-  md2$re- md2$un

ggplot(md2, aes(x=group, y=diff, fill=dir)) +geom_bar(stat="identity")


t.test(md2$un, md2$re)
...
ggplot(md, aes(y=pair, x=rfagepred))+
  ...
  xlab("pair") +
  xlab("Age (weeks) from marker gene expression (random forest model)")+
  scale_colour_gradient2(low="red2",mid="yellow",midpoint=70, high="forest green")+
  theme_bw()+
