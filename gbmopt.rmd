...
vrimp <- summary(mymodel)


vrimp$sname <- substr(vrimp$var,1,2)
vrimp$sname <- factor(vrimp$sname, levels=vrimp$sname[order(vrimp$rel.inf, decreasing=T)])

(p=ggplot(vrimp, aes(x=sname, y=rel.inf))+
  theme_bw()+
...
md <- md %>%
  ...
  left_join(select(vrimp, -sname), by= c("marker"="var"))
