...
=======
load(file="modelgbm.rdata")
>>>>>>> 2abd2310efbabb335c5333aebd2a3be56a81ff82
...
sm4 <- select(sm3,  predage, predagediff, predagecontrol, group, perm, cm) %>%
  filter(predagediff !=0)%>%
  left_join(rvals, by="group") %>%
    left_join(mameans, by="cm") %>%
  mutate(ma= substr(cm, 1,2), 
         dir = meandiff >0 ) %>%
  left_join(vrimprf, by=c("ma"= "sname", "cm"="name"))

sm5 <-mameans %>%
#  left_join(vrimprf, by=c("cm"="name")) %>%
  
  left_join(vrimpgbm, by=c("cm"="var")) %>%
  mutate(dir = meandiff >0 )
...
sm4$ma <- factor(sm4$ma, levels=unique(sm4$ma[order(sm4$meandiff)]))

ggplot(sm4, aes(x=ma, y=predagediff, fill=dir))+
...
sm5$sname <- factor(sm5$sname, levels=unique(sm5$sname[order(sm5$meandiff)]))

ggplot(sm5, aes(x=sname, y=meandiff, fill=dir, size=rel.inf))+
  geom_point(shape=21)+
  geom_hline(yintercept = 0)+
#  facet_wrap(~cm)+
  theme_bw()+
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
save(sm3, sm4, sm5, file="permsrf.rdata")

save(sm3, sm4, sm5, file="permsgbm.rdata")
````
...
ggplot(msummar, aes(y=cmmeandiff, x= ma,size=Overall, fill=dir))+
  geom_point(shape=21)+
...
>>>>>>> 2abd2310efbabb335c5333aebd2a3be56a81ff82
  ...
  ggtitle("Which markers rejuvenate?", 
          subtitle= paste0("Model type: ", toupper(mymodel$method), ", ", length(mymodel$coefnames), " markers used\n", 
                           nperm, " permutations"))+
<<<<<<< HEAD
  geom_smooth(aes(y=cmmeandiff, x=Overall), inherit.aes=F, method="lm", se=F)
=======
